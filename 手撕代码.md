...
4.  模拟实现对象的深拷贝(six)
    ...
    1. JSON 方法
       ...
       1. 对象中有循环引用无法正确实现深拷贝
       2. 会丢失函数和 undefined
       3. 如果 obj 里有正则、error 对象，序列化的结果将只得到空对象
       4. obj 中有 NaN,Infinity 或者-Infinity,会变成 null
    ...
    1. ES6 flat 实现
       ...
       function myFlat(arr, depth = 1) {
         return depth > 0
           ? arr.reduce((pre, cur) => {
               return pre.concat(
                 Array.isArray(cur) ? myFlat(cur, depth - 1) : cur
               );
             }, [])
           : arr.slice();
       }
...
8.  模拟实现 Array.fill(), Array.filter()(six)
...
9.  模拟实现 Array.find(), Array.findIndex()(six)
...
16. setTimeout 模拟实现 setInterval
    ...
    function currying(fn, ...args) {
      ...
      const res = (...newArgs) => {
        ...
        if (allArgs.length === length) {
          ...
          // 这里不能传入参数, 因为前面的allArgs已经保存了之前的参数，不需要传入
          return res;
        }
      };
      return res;
...
23. 实现 add()
    ...
    function add() {
      ...
      let fn = function () {
        // return一定要加
        return add.apply(this, args.concat([...arguments]));
      };
...
29. Promise
    ...
    2. setTimeout 封装成 promise(sleep)
    ...
    4. promisify 方法
       传入一个遵循常见的错误优先的回调风格的函数(即以(err, data) => ... 回调作为最后一个参数)，并返回一个返回 Promise 的版本
    ...
    14. promise 的一些问题
        ...
        【建议星星】要就来 45 道 Promise 面试题一次爽到底(1.1w 字用心整理)
        https://juejin.cn/post/6844904077537574919

        1. 异步任务时先执行 then 再改变状态 resolve
        ...
        1. 抛出错误, result 状态变为失败
        1. return 一个非 promise 对象, 例如 return 123 , result 为成功的 promise
...
30. async/await 封装成 promise

    字节提前批前端面试题：将这段 async/await 代码翻译成 Promise
    https://mp.weixin.qq.com/s?__biz=MzU5NDM5MDg1Mw==&mid=2247487994&idx=1&sn=352024fff02977961196407d59d7f039&chksm=fe00abd0c97722c61cf84a43217c6de424e427fe1f8c088e0653c3f12d9c80c4a3b5e4d444c9&token=828996357&lang=zh_CN#rd
...
35. 实现 `a == 1 && a == 2 && a == 3`和`a === 1 && a === 2 && a === 3`
...
40. 大数相加
    ...
    let b = "1234567899999999999"; // 19位

    console.log(parseInt(a) + parseInt(b)); // 1243575099254741000,错误, 超过16个十进制则无法正确表示精度
    function add(a, b) {
      let res = "";
    ...
    let res = add(a, b);
    console.log(res, "res"); // 1243575099254740990
    ```
...
43. DOM2JSON 函数
    ...
    function dom2Json(domTree) {
      ...
      Array.prototype.slice
        .call(domTree.children)
        .forEach((child) => obj.children.push(dom2Json(child)));
      return obj;
    }
    ```

44. JSON2DOM 函数-虚拟 DOM 转为真实 DOM
...
46. 怎么在制定数据源里面生成一个长度为 n 的不重复随机数组 能有几种方法 时间复杂度多少（字节）
...
50. 以广度优先搜索 dom 树返回第一个 className 为 container 的 dom 节点
...
51. 打乱数组
    ...
    var disruptArr = function (arr) {
      let res = [];

      for (let i = arr.length - 1; i >= 0; i--) {
        let index = Math.floor(Math.random() * arr.length);
        let number = arr.splice(index, 1)[0];
        res.push(number);
      }
      return res;
    };
    let res = disruptArr(arr);
    console.log(res, "res");
    ```
...
55. 拼手气分红包
    ...
    ```js
    // 拼手气分红包
    // 要求传入money金钱和num人数,最少得一分
    function redPackage(money, num) {
      let arr = [];
      let total = money;
      let n = num;
      for (let i = 0; i < n - 1; i++) {
        let random = Number((Math.random() * total).toFixed(2));
        if (random < 0.01) {
          random = 0.01;
        }
        arr.push(random);
...
57. 说出下面的运行结果
    ...
    3. 立即执行函数 - if 的作用域问题
       ...
       (function () {
         console.log(name, "name"); // undefined 'name'
         // 因为变量提升, if语句是不会产生作用域的, 所以在立即执行函数的最上方会定义 var name = undefined
         // var name = undefined
