...
1. 下一个排列
   ...
   var nextPermutation = function (nums) {
     let len = nums.length;
     let i = len - 2;
     // 从后向前查找第一个相邻升序的元素(i,i+1), 说明i后面一定是降序
     while (i >= 0 && nums[i] >= nums[i + 1]) i--;
     // 说明此时不是最大的排列，要去寻找i+1后面比nums[i]大的最小值
     // 如果i=-1的话, 不存在 nums[-1] 所以无法调换位置
     if (i >= 0) {
       // 找到i后面最小的比nums[i] 大的数字下标j
       let j = len - 1;
       // 因为是降序,从后向前查找满足>nums[i]的第一个元素
...
2. 寻找重复数-使用快慢指针(和环形链表一个概念)
   ...
   var findDuplicate = function (nums) {
     ...
     while (true) {
       ...
       fast = nums[nums[fast]];
       if (slow === fast) {
         // slow和fast相等，则其中一个置为0，两个同步速度向前进
         let _slow = 0;
...
1. 移掉 K 位数字
   ...
   var removeKdigits = function (num, k) {
     ...
     for (let i = 0; i < num.length; i++) {
       let c = num[i];
       // 维护一个单调递增栈
       // 如果c比栈顶元素更小, 则弹出栈顶元素，移除的数量k--
       while (k > 0 && stack.length && c < stack[stack.length - 1]) {
         stack.pop();
         // k表示要移除的数量
         k--;
       }
       // 排除掉 0在栈顶的情况(c === '0' && stack.legnth === 0)
...
19. 链表排序 | 排序链表(中)(难) - 使用归并排序
    ...
    var sortList = function (head) {
      ...
      // 为什么要这一步? 一分为二 变成head开头的节点和slow开头的节点
      preSlow.next = null;
...
23. K 个一组翻转链表(难)
    ...
    const reverseList = (head, tail) => {
      ...
      let pre = tail.next;
      let cur = head;
      // 终止条件 pre == tail尾节点, 说明前面的节点已经全部逆序成功
      while (pre !== tail) {
        const temp = cur.next;
        cur.next = pre;
        pre = cur;
...
24. 合并 K 个升序链表(难)
    ...
    function mergeList(list, start, end) {
      // 如果 start === end 说明分治的分到头了，只剩一组元素了，直接返回作为leftList或者rightList即可
      if (start === end) {
        return list[start];
      }
...
3. 不同路径 II
   ...
   var uniquePathsWithObstacles = function (obstacleGrid) {
     ...
     const dp = Array(m)
       .fill()
       .map((item) => Array(n).fill(0));
     // i < m && obstacleGrid[i][0] === 0 注意
     // 当i<m并且obstacleGrid[i][0] === 0，循环就停止了
...
9. 划分为 k 个相等的子集
   ...
   var canPartitionKSubsets = function (nums, k) {
     ...
     // 降序排序，优化尽快命中剪枝
     nums.sort((a, b) => b - a);
     var getPath = function (bucket, index) {
       if (index === nums.length) {
         for (let i = 0; i < k; i++) {
       ...
       for (let i = 0; i < k; i++) {
         ...
         if (getPath(bucket, index + 1)) return true;
         bucket[i] -= nums[index];
       }
       return false;
     };
     let bucket = new Array(k).fill(0);
     return getPath(bucket, 0);
   };
   ```

10. 零钱兑换 2(难) - 完全背包求组合数
...
14. 打家劫舍一二三 | 房屋偷盗
    ...
    var rob = function (nums) {
      ...
      let len = nums.length;
      // robRange函数不包括 nums.length === 0或1的时候, 所以需要特别举例出
      if (len === 0) return 0;
...
17. 最长递增子序列的个数
    ...
    var lengthOfLIS = function (nums) {
      ...
      // 初始化，对应的dp[i]起始大小至少都是1
      let dp = new Array(nums.length).fill(1);
      for (let i = 1; i < nums.length; i++) {
        for (let j = 0; j < i; j++) {
          // 位置i的最长升序子序列=j从0到i-1各个位置的最长升序子序列+1的最大值，即取dp[j]+1的最大值
          if (nums[i] > nums[j]) {
            dp[i] = Math.max(dp[i], dp[j] + 1);
          }
          // 取所有dp[i]中最长的长度
    ...
    var findNumberOfLIS = function (nums) {
      ...
      for (let i = 1; i < nums.length; i++) {
        for (let j = 0; j < i; j++) {
          if (nums[i] > nums[j]) {
            // 如果dp[j]+1 > dp[i],更新dp[i]和count[i]
            if (dp[j] + 1 > dp[i]) {
              dp[i] = dp[j] + 1;
              count[i] = count[j];
            } else if (dp[j] + 1 === dp[i]) {
              // 以i为结尾的子串的最长递增子序列的个数+以j为结尾的子串的最长递增子序列的个数
              count[i] += count[j];
            }
          }
          // 记录最长递增子序列的长度
      ...
      for (let i = 0; i < nums.length; i++) {
        // 以nums[i]为结尾的字符串，最长递增子序列的长度为maxCount的个数求和
        if (dp[i] === maxCount) {
          res += count[i];
        }
      }
...
37. 字符串交织 | 交错字符串
    ...
    var isInterleave = function (s1, s2, s3) {
      ...
      let dp = new Array(m + 1).fill().map((item) => Array(n + 1).fill(false));
      dp[0][0] = true;
      for (let i = 1; i <= m && s1[i - 1] === s3[i - 1]; i++) {
        dp[i][0] = true;
      }
      for (let j = 1; j <= n && s2[j - 1] === s3[j - 1]; j++) {
        dp[0][j] = true;
      }
      for (let i = 1; i <= m; i++) {
        for (let j = 1; j <= n; j++) {
...
40. 解码方法 - 与`把数字翻译成字符串`比较
    ...
    var numDecodings = function (s) {
      ...
      for (let i = 2; i <= s.length; i++) {
        // 判断s.slice(i,i+2)是否在1-26之中
        let str = s.slice(i - 2, i);

        // s[i-2]和s[i-1]组合编码, 排除掉'05'这种不能组合的情况
        if (+str <= 26 && +str >= 10) {
          dp[i] = dp[i - 2];
        }
        // s[i-1]单独做编码又是一种新的解码方式
        if (+str[1] >= 1 && +str[1] <= 9) {
          dp[i] += dp[i - 1];
        }
      }
...
45. 最长定差子序列
    ...
    var longestSubsequence = function (arr, difference) {
      // 状态转移方程为
      // dp[num] = dp[num - difference] + 1;  // 情况1
      // dp[num] = 1                          // 情况2
      let res = 0;
      const dp = new Map();
      for (let i = 0; i < arr.length; i++) {
...
46. 单词拆分 - 完全背包经典题
    ...
    var wordBreak = function (s, wordDict) {
      ...
      // 第一个一定要设置true, 才能使后面的根据前面的设置为true
      dp[0] = true;
      for (let i = 0; i <= s.length; i++) {
        for (let j = 0; j < wordDict.length; j++) {
          // 前提是s[i] >= wordDict[j].length
          if (i >= wordDict[j].length) {
            // s.slice(i-wordDict[j].length, i) === wordDict[j] 并且 之前的为true
            if (
...
48. 编辑距离
    ...
    var minDistance = function (word1, word2) {
      ...
      let dp = new Array(m + 1).fill().map((item) => Array(n + 1).fill(0));
...
- for 循环横向遍历， 递归纵向遍历
- 题目分类: 组合(2,3,4,5)，子集(6,7)， 排列(8,9,10)， 分割(9,10)， 棋盘问题， 其它(递增子序列， 重新安排行程)
...
1. 电话号码的字母组合
   ...
   var letterCombinations = function (digits) {
     ...
     var getPath = function (path, index) {
       ...
       for (let num of numberList[digits[index]]) {
         path.push(num);
...
4. 组合总和 | 允许重复选择元素的组合
   ...
   var combinationSum = function (candidates, target) {
     let res = [];
     var getPath = function (path, sum, index) {
...
5. 组合总和 II | 含有重复元素集合的组合(难)
   ...
   var combinationSum2 = function (candidates, target) {
     ...
     var getPath = function (path, sum, index) {
       ...
       let f = "";
       for (let i = index; i < candidates.length; i++) {
         if (f === candidates[i]) continue;
         path.push(candidates[i]);
         f = candidates[i];
         sum += candidates[i];
         getPath(path, sum, i + 1);
         path.pop();
         sum -= candidates[i];
       }
     };
    ...
    var partition = function (s) {
      ...
      var getPath = function (path, index) {
        ...
        for (let i = index; i < s.length; i++) {
          ...
          // 表示第index个位置，取i-index+1个字符
          path.push(s.substr(index, i - index + 1));
          // 或者使用下面这种，取index到i的字符串
    ...
    var partition = function (s) {
      ...
      var getPath = function (path, i) {
        ...
        for (let j = i; j < s.length; j++) {
          ...
          // 需要考虑 i===j的时候，所以必须j-i+1, 至少是一个字符串
...
13. 复原 IP 地址(难)
    ...
    var restoreIpAddresses = function (s) {
      ...
      var getPath = function (path, i) {
        ...
        for (let j = i; j < s.length; j++) {
          // 表示从i开始，取j-i+1个元素, 要求至少有一个元素,否则str为空，会循环爆栈
          let str = s.substr(i, j - i + 1);
          // 也可以用continue
          // 排除 >255的情况
    ...
    */
    var numsSameConsecDiff = function(n, k) {
      ...
      var getPath = function(path) {
        ...
        for (let i=path.length ? 0:1; i<=9; i++) {
          let pre  = path[path.length-1]
          if (!path.length || pre - i === k || i - pre ===k) {
            path.push(i)
            getPath(path)
            path.pop()
          }
    ...
    var numsSameConsecDiff = function (n, k) {
      ...
      var getPath = function (path) {
        ...
        for (let i = 0; i <= 9; i++) {
          // 当path为空的时候,pre为undefined, undefined - i 为NaN
          let pre = path[path.length - 1];
          if (path.length === 0 || pre - i === k || i - pre === k) {
            path.push(i);
            getPath(path);
            path.pop();
          }
...
23. 打印从 1 到最大的 n 位数-考虑大数(1. 回溯 2. 位运算)(难)
...
6. 跳跃游戏 II
   ...
   var jump = function (nums) {
     ...
     // 注意i的取值范围
     for (let i = 0; i < nums.length - 1; i++) {
       // 当前索引可覆盖的范围和之前nextIndex进行比较
       // 求下一步的最大索引
       nextIndex = Math.max(nums[i] + i, nextIndex);
       // 假如移动下标已经遇到当前距离最远覆盖下标，步数+1
       if (i === curIndex) {
         curIndex = nextIndex;
         step++;
       }
   ...
   var longestSubarray = function (nums, limit) {
     ...
     for (let right = 0; right < nums.length; right++) {
       ...
       if (queMax.length && queMin.length && queMax[0] - queMin[0] > limit) {
         // 如果滑动窗口的左边left++, 就需要把对应的两个队列中的元素删除
   ...
   var minWindow = function (s, t) {
     ...
     for (let right = 0; right < s.length; right++) {
       ...
       if (need[str]) {
         // 更新窗口统计
         window[str] = (window[str] || 0) + 1;
         // 当前窗口和需要的字符匹配时，验证数量+1
         if (window[str] == need[str]) {
           valid++;
         }
       }
       // 当验证数量和需要的字符个数相同时,开始收缩窗口
...
3. 二分图
   ...
   var isBipartite = function (graph) {
     ...
     const traverse = (graph, v) => {
       // 如果已经确定不是二分图了，就不用浪费时间再递归遍历了, 直接return停止递归
       if (!flag) return;
       // 记录当前节点v已经访问过了
       visited[v] = true;
       // 遍历当前节点所连接的边的另一个节点
...
12. 求平方根
    ...
    var mySqrt = function (x) {
      ...
      while (left < right) {
        // 必须是ceil,不能是floor
        // 因为当x=8时，存在left=2,right=3的情况不停重复，会超出时间限制
...
15. 寻找旋转排序数组中的最小值(与上一题类似,但是元素值互不相同)
...
3. 有序数组中差绝对值之和
   ...
   var getSumAbsoluteDifferences = function (nums) {
     // 利用前缀和
     // nums[i]与其后所有元素的绝对值之和 === 其后所有元素总和减去其后元素个数与nums[i]的乘积，
     // nums[i]其前所有元素绝对值之和 === nums[i]*其前元素个数 - 其前元素总和
     let res = [];
     let sum = nums.reduce((pre, cur) => pre + cur);
     let curSum = 0;
     for (let i = 0; i < nums.length; i++) {
...
9. 数组中出现次数超过一半的数字 | 多数元素-摩尔投票(抵消法)
...
1. 二叉树中的最大路径和
   ...
   var maxPathSum = function (root) {
     ...
     var dfs = function (node) {
       ...
       // 当前节点子树内部的最大路径和(节点值+左子树最大和+右子树最大和)
   ...
   var firstMissingPositive = function (nums) {
     ...
     for (let i = 0; i < len; i++) {
       while (nums[i] >= 1 && nums[i] <= len && nums[i] !== nums[nums[i] - 1]) {
         // num[index] 放到对应的 num[num[index]-1]的位置上
         // 把num[i]放到对应的nums[nums[i]-1]的位置上
...
10. 合并两个有序数组(难)
    ...
    var merge = function (nums1, m, nums2, n) {
      // 因此如果我们采用正向遍历时，nums2 的数值替换到 nums1 上时，如果此时 nums1 上该数字还没有遍历到，则会导致该数字丢失。
      let index = m + n - 1;
      let i = m - 1,
        j = n - 1;
      while (i >= 0 && j >= 0) {
...
16. 比较版本号
    ...
    arr.sort((a, b) => {
      ...
      while (true) {
        ...
        const s2 = arr2[i];
        i++;
        // 先判断长度不相等的情况,比如4.2和4.2.1，前面都相等，只能比较长度
...
28. 旋转图像
    ...
    var rotate = function (matrix) {
      ...
      // 左上角的位置移到右上角，右上角移到右下角，右下角移到左下角，左下角移到左上角的位置
      let n = matrix.length;
...
30. K 个一组翻转链表
    ...
    var reverseKGroup = function (head, k) {
      ...
      while (head) {
        ...
        const cur = tail.next;
        // head还是之前的head节点，tail还是之前的tail节点
        [head, tail] = reverseList(head, tail);
        // 把子链表重新接回原链表
        pre.next = head;
        tail.next = cur;
...
32. 字符串解码
    ...
    var decodeString = function (s) {
      ...
      for (let i = 0; i < s.length; i++) {
        ...
        // 如果c为数字的情况
        if (!isNaN(c)) {
          // 考虑k为两位数及以上的情况， 例如 12[c]
          num = num * 10 + Number(c);
        } else if (c === "[") {
          // 把[前的数字入栈，把之前的字符串保存起来，入栈
          numStack.push(num);
          num = 0;
          ...
          // 数字出栈
          // 结果为 原来的字符串(存在strStack中) + 现有的result*数字
...
35. 最大数
    ...
    var largestNumber = function (nums) {
      // 因为要是数字尽可能大, [10, 2] 比较 102和210哪个更大
      // 自定义排序
      let res = nums.sort((a, b) => b + "" + a - (a + "" + b)).join("");
      // 排除掉 [0,0]的情况，因为排序后会返回'00'
